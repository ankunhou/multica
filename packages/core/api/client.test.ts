...
describe("ApiClient", () => {
  ...
  it("resolves relative upload URLs against the API base URL", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(
        new Response(
          JSON.stringify({
            ...
            avatar_url: "/uploads/users/user-1/avatar.png",
          }),
    ...
    expect(user.avatar_url).toBe("https://api.example.test/uploads/users/user-1/avatar.png");
  });

  it("keeps relative upload URLs relative when no API base URL is configured", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(
        new Response(
    ...
    expect(user.avatar_url).toBe("/uploads/users/user-1/avatar.png");
  });
