...
export class ApiClient {
  ...
  private resolveUploadUrl(value: string): string {
    if (!value.startsWith("/uploads/") || this.baseUrl === "") return value;
    return `${this.baseUrl.replace(/\/$/, "")}${value}`;
  }

  private normalizeResponseUrls(value: unknown): unknown {
    if (typeof value === "string") return this.resolveUploadUrl(value);
    if (Array.isArray(value)) return value.map((item) => this.normalizeResponseUrls(item));
    if (!value || typeof value !== "object") return value;

    return Object.fromEntries(
      Object.entries(value).map(([key, entry]) => [key, this.normalizeResponseUrls(entry)]),
    );
  }
  ...
  private async fetch<T>(path: string, init?: RequestInit, options?: FetchOptions): Promise<T> {
    ...
    return this.normalizeResponseUrls(body) as T;
  }
