...
type AgentTranscriptT = ReturnType<typeof useT<"agents">>["t"];
...
function getEventLabel(item: TimelineItem, t: AgentTranscriptT): string {
  switch (item.type) {
    case "text":
      return t(($) => $.transcript.event_agent);
    case "thinking":
      return t(($) => $.transcript.event_thinking);
    case "tool_use":
      return item.tool ?? t(($) => $.transcript.event_tool);
    case "tool_result":
      return item.tool ? `${item.tool}` : t(($) => $.transcript.event_result);
    case "error":
      return t(($) => $.transcript.event_error);
    default:
      return t(($) => $.transcript.event_event);
  }
}
      ...
      {segments.map((seg) => {
        ...
        return (
          <button
            ...
            title={`${getEventLabel(items[seg.startIdx]!, t)}${seg.count > 1 ? t(($) => $.transcript.more_events_suffix, { count: seg.count - 1 }) : ""}`}
          >
        ...
        <div className="flex items-start gap-2 px-4 py-2">
          ...
          >
            <div className="flex items-start gap-1.5">
              ...
              <span className="truncate">{summary || t(($) => $.transcript.empty_summary)}</span>
            </div>
          </CollapsibleTrigger>
