...
// the verdict landed.
function activeTimeText(task: AgentTask, relativeTime: (value: string | Date) => string): string {
  if (task.status === "running" && task.started_at) {
    return relativeTime(task.started_at);
  }
  if (task.status === "dispatched" && task.dispatched_at) {
    return relativeTime(task.dispatched_at);
  }
...
function ActiveRow({ task, issueId }: { task: AgentTask; issueId: string }) {
  ...
  const time = activeTimeText(task, relativeTime);
