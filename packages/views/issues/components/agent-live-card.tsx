...
import { TranscriptButton, buildTimeline, type TimelineItem } from "../../common/task-transcript";
      ...
      .then(({ tasks }) => {
        ...
        setTaskStates((prev) => {
          ...
          for (const task of tasks) {
            ...
            next.set(task.id, existing ? { task, items: existing.items } : { task, items: [] });
          }
          return next;
        });
    ...
    <>
      ...
      <div className="mt-4 sticky top-4 z-10 rounded-lg bg-background/80 supports-[backdrop-filter]:bg-background/55 backdrop-blur-md">
        <SingleAgentLiveCard
          ...
          issueId={issueId}
          agentName={
            firstEntry.task.agent_id
              ? getActorName("agent", firstEntry.task.agent_id)
...
function SingleAgentLiveCard({ task, items, issueId, agentName }: SingleAgentLiveCardProps) {
  ...
  return (
    <div
      className={
        isQueued
          ? "rounded-lg border border-agent-queued/25 bg-agent-queued/5"
          : "rounded-lg border border-agent-running/20 bg-agent-running/5"
      }
    >
      <div className="flex items-center gap-2 px-3 py-2 text-muted-foreground">
        {task.agent_id ? (
          ...
          <div className="flex items-center justify-center h-5 w-5 rounded-full shrink-0 bg-agent-running/10 text-agent-running">
            <Bot className="h-3 w-3" />
          </div>
        )}
        <div className="flex items-center gap-1.5 text-xs min-w-0">
          {isQueued ? (
            <Clock className="h-3 w-3 text-agent-queued shrink-0" />
          ) : (
            <Loader2 className="h-3 w-3 animate-spin text-agent-running shrink-0" />
          )}
          ...
          <span className="text-muted-foreground tabular-nums shrink-0">
            {isQueued ? t(($) => $.agent_live.queued_elapsed_prefix, { elapsed }) : elapsed}
          </span>
          {!isQueued && toolCount > 0 && (
            <span className="text-muted-foreground shrink-0">
              {t(($) => $.agent_live.tool_count, { count: toolCount })}
            </span>
          )}
        </div>
