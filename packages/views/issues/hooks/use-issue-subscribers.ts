...
import type { SubscriberAddedPayload, SubscriberRemovedPayload } from "@multica/core/types";
...
export function useIssueSubscribers(issueId: string, userId?: string) {
  ...
  const { data: subscribers = [], isLoading: loading } = useQuery(issueSubscribersOptions(issueId));
    ...
    useCallback(
      (payload: unknown) => {
        ...
        qc.setQueryData<IssueSubscriber[]>(issueKeys.subscribers(issueId), (old) => {
          if (!old) return old;
          if (old.some((s) => s.user_id === p.user_id && s.user_type === p.user_type)) return old;
          return [
            ...old,
            {
              issue_id: p.issue_id,
  ...
  useWSEvent(
    ...
    useCallback(
      (payload: unknown) => {
        ...
        if (p.issue_id !== issueId) return;
        qc.setQueryData<IssueSubscriber[]>(issueKeys.subscribers(issueId), (old) =>
          old?.filter((s) => !(s.user_id === p.user_id && s.user_type === p.user_type)),
        );
      },
      [qc, issueId],
    ),
  ...
  const isSubscribed = subscribers.some((s) => s.user_type === "member" && s.user_id === userId);
