...
export function ActivityHeatmap({ usage }: { usage: RuntimeUsage[] }) {
  ...
  const { cells, monthLabels, insights } = useMemo(() => {
    ...
    for (const c of cellsWithLevel) {
      ...
      if (month !== lastMonth && c.dayOfWeek === 0) {
        months.push({
          label: monthFormatter.format(new Date(c.date + "T00:00:00")),
          week: c.week,
        });
        lastMonth = month;
      }
    ...
    weekdayAvg.forEach((avg, i) => {
      const name = weekdayFormatter.format(new Date(2024, 0, 7 + i));
      if (avg > busyDayAvg) {
        busyDayAvg = avg;
        busyDayName = name;
      }
  ...
  }, [usage, pricings, weekdayFormatter, monthFormatter]);
  ...
  return (
    <div className="space-y-4">
      ...
      <InsightsRow insights={insights} formatDate={shortDate} />
    </div>
  );
}
  ...
  return (
    <dl className="grid grid-cols-2 gap-x-4 gap-y-3 border-t pt-3 sm:grid-cols-4">
      <Insight
        ...
        value={busiestDay ? formatDate(`${busiestDay.date}T00:00:00`) : "—"}
