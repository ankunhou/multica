...
// trailing `-dirty`). Daemons built from source (`just build` / `just daemon`)
// report this shape; tagged releases are bare semver. Treating dev-described
// daemons as OK keeps `just daemon` unblocked without weakening the
