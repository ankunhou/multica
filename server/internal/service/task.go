...
func (s *TaskService) notifyQuickCreateFailed(ctx context.Context, task db.AgentTaskQueue, qc QuickCreateContext, errMsg string) {
	...
	if user, err := s.Queries.GetUser(ctx, requesterID); err == nil && user.Language.Valid {
		locale = user.Language.String
	}
	if errMsg == "" {
		errMsg = i18n.QuickCreateDefaultFailure(locale)
	}
	details, _ := json.Marshal(map[string]any{
		"task_id":         util.UUIDToString(task.ID),
		"agent_id":        util.UUIDToString(task.AgentID),
	...
	item, err := s.Queries.CreateInboxItem(ctx, db.CreateInboxItemParams{
		...
		IssueID:       pgtype.UUID{},
		Title:         i18n.QuickCreateFailedTitle(locale),
