...
func TestBuildInvitationParams_ZhHans(t *testing.T) {
	p := buildInvitationParams(
		"noreply@multica.ai",
		"invitee@example.com",
		"Alice",
	...
	if p.Subject != "Alice 邀请你加入 Multica 工作区 Acme" {
		t.Errorf("unexpected subject: %q", p.Subject)
	}
	if !strings.Contains(p.Html, "接受邀请") {
		t.Errorf("body missing localized CTA: %s", p.Html)
	}
}
