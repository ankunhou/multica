...
func (h *Handler) CreateInvitation(w http.ResponseWriter, r *http.Request) {
	...
	if h.EmailService != nil && workspaceName != "" {
		...
		if existingUser.Language.Valid {
			locale = existingUser.Language.String
		}
		invID := uuidToString(inv.ID)
		go func() {
			if err := h.EmailService.SendInvitationEmail(email, inviterName, workspaceName, invID, locale); err != nil {
