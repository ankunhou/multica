...
func TestAutopilotFailureMonitor_PausesOffenderAndNotifiesCreator(t *testing.T) {
	...
	if got := item["title"]; got != "Autopilot paused: Failure monitor: offender" {
...
func TestAutopilotFailureMonitor_LocalizesNotificationByRecipientLanguage(t *testing.T) {
	queries := db.New(testPool)
	bus := events.New()
	cfg := failureMonitorConfig{
		Interval:  time.Hour,
	...
	if _, err := testPool.Exec(context.Background(), `UPDATE "user" SET language = 'zh-Hans' WHERE id = $1`, testUserID); err != nil {
	...
	agentID := pickFixtureAgent(t)
	ap := seedAutopilot(t, queries, "Failure monitor: localized", "member", parseUUID(testUserID), agentID)
	...
	if len(inboxEvents) != 1 {
		t.Fatalf("expected 1 inbox event, got %d", len(inboxEvents))
	}
	item := inboxEvents[0].Payload.(map[string]any)["item"].(map[string]any)
	if got := item["title"]; got != "自动化已暂停：Failure monitor: localized" {
		t.Fatalf("expected localized title, got %v", got)
	}
	body := ""
	...
	if !strings.Contains(body, "过去 7 天内") {
		t.Fatalf("expected localized body, got %v", item["body"])
	}
}
