...
func emitAutopilotPausedNotifications(
	...
	for _, r := range recipients {
		...
		title := i18n.AutopilotPausedTitle(r.Language, autopilot.Title)
		body := i18n.AutopilotPausedBody(r.Language, candidate.FailedRuns, candidate.TotalRuns, failPct, cfg.Lookback)
...
func resolveAutopilotPausedRecipients(
	...
	if autopilot.CreatedByType == "member" {
		return []pausedRecipient{{
			Type:     "member",
			ID:       autopilot.CreatedByID,
			Language: userLanguage(ctx, queries, autopilot.CreatedByID),
		}}
	}
	...
	return []pausedRecipient{{
		Type:     "member",
		ID:       member.UserID,
		Language: userLanguage(ctx, queries, member.UserID),
	}}
}

func userLanguage(ctx context.Context, queries *db.Queries, userID pgtype.UUID) string {
