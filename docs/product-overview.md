...
- **基本信息**：名字、描述、头像（自动生成）
- **Provider**：选择底层是 Claude / Codex / GitHub Copilot CLI / OpenClaw / OpenCode / Hermes / Gemini / Pi / Cursor / Kimi / Kiro 中的哪一个
...
- **在线**：15 秒一次心跳
- **离线**：如果 server 超过 150 秒没有收到有效心跳，把 runtime 标记为离线（server 后台 sweeper 每 30 秒巡检，最坏约 3 分钟内发现）
...
| `multica daemon status` | 打印 daemon 状态、探测到的 agent、当前跟踪的 workspace 数量 |
...
`agent_runtime`, `daemon_token`, `daemon_connection`（早期兼容表）, `task_usage`, `task_usage_daily`, `task_usage_rollup_state`, `task_usage_daily_dirty`
...
每次触发都在 `autopilot_run` 里留一条记录：`issue_created → running → completed/failed/skipped`。在 UI 的 autopilot 详情页可以看全部历史。
...
```bash
multica workspace list | get | members | update
...
```bash
multica agent list | get | create | update | archive | restore | tasks | avatar
multica agent skills list | set
multica skill list | get | create | update | delete | import
multica skill files list | upsert | delete
multica autopilot list | get | create | update | delete | trigger | runs
multica autopilot trigger-add | trigger-update | trigger-delete
multica project list | get | create | update | delete | status
multica project resource list | add | remove
...
共 **40+ 张表**，覆盖 10 个产品域。以下按域列出最重要的字段，供文案/产品查询"某个功能背后到底存了什么"。
...
- `autopilot_run` — 执行记录（status: issue_created/running/skipped/completed/failed）
...
- `task_usage_daily` / `task_usage_rollup_state` / `task_usage_daily_dirty` — 运行时按日聚合 token 用量（给计费/容量规划用）
