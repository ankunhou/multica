...
export function LandingHeader({ variant = "dark" }: { variant?: "dark" | "light" }) {
  const { t } = useLocale();
    ...
    >
      <div className="mx-auto flex h-[76px] max-w-[1320px] items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/" className="flex items-center gap-3">
          <MulticaIcon
            className={cn("size-5", variant === "dark" ? "text-white" : "text-[#0a0d12]")}
            noSpin
          />
          <span
            className={cn(
        ...
        <div className="flex items-center gap-2.5 sm:gap-3">
          <Link
            href="/changelog"
            className={cn(headerButtonClassName("ghost", variant), "hidden sm:inline-flex")}
          >
            {t.header.changelog}
          </Link>
          <Link
          ...
          <Link href={user ? "/" : "/login"} className={headerButtonClassName("solid", variant)}>
