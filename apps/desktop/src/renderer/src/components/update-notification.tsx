  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-80 rounded-lg border border-border bg-background p-4 shadow-lg animate-in slide-in-from-bottom-2 fade-in duration-300">
      <button
        ...
        aria-label={t(($) => $.updates.notification.dismiss)}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium">
              {t(($) => $.updates.notification.available_title)}
            </p>
            <p className="text-xs text-muted-foreground mt-0.5">
              {t(($) => $.updates.notification.available_description, {
            ...
            >
              {t(($) => $.updates.notification.download)}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="mt-2 flex items-center gap-1.5">
              ...
              >
                {t(($) => $.updates.notification.see_changes)}
              </button>
              <button
                onClick={handleInstall}
                className="inline-flex items-center rounded-md bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors"
              >
                {t(($) => $.updates.notification.restart_now)}
              </button>
            </div>
          </div>
        </div>
