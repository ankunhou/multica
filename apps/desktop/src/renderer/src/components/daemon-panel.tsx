      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3">
          ...
          <button
            ...
            aria-label={t(($) => $.daemon.logs.close)}
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 border-b px-4 py-2">
          ...
          <div className="relative w-56">
            ...
            <input
              ...
              placeholder={t(($) => $.daemon.logs.search_placeholder)}
        ...
        <div className="flex shrink-0 items-center justify-between border-t bg-muted/30 px-4 py-1.5 text-xs text-muted-foreground">
          <span className="tabular-nums">
            {t(($) => $.daemon.logs.showing, {
              filtered: filtered.length,
              total: logs.length,
            })}
            {logs.length === MAX_LOG_LINES && (
              <span className="ml-1 text-muted-foreground/60">
                {t(($) => $.daemon.logs.buffer_full)}
              </span>
            )}
          </span>
          {!autoScroll && (
...
function EmptyState({
  ...
  let subtitle: string;
  if (hasFilter) {
    title = t(($) => $.daemon.logs.empty_filtered);
    subtitle = t(($) => $.daemon.logs.empty_filtered_hint);
  } else if (!isRunning) {
    title = t(($) => $.daemon.logs.empty_not_running);
    subtitle = t(($) => $.daemon.logs.empty_not_running_hint);
  } else if (!hasLogs) {
    title = t(($) => $.daemon.logs.empty_running);
    subtitle = t(($) => $.daemon.logs.empty_running_hint);
  } else {
    title = "";
    subtitle = "";
  }
