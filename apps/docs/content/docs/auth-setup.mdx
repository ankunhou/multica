...
Production deployments should leave `MULTICA_DEV_VERIFICATION_CODE` empty and set `APP_ENV=production`. If you deploy via `just selfhost` / `docker-compose.selfhost.yml`, `APP_ENV` defaults to `production`.
