...
Multica CLI 把 Web UI 能做的事几乎全部搬到了命令行上（创建 [议题](/issues)、分配 [智能体](/agents)、启动 [守护进程](/daemon-runtimes) 等等）。这一页把所有顶级命令列出来，每条配一句用途。完整 flag 和示例用 `multica <命令> --help` 查。
...
<Callout type="info">
`list` 类命令（`multica issue list`、`autopilot list`、`project list` 等）表格里默认显示**可直接复制**的短 ID：议题是 key（如 `MUL-123`），其余资源是 UUID 短前缀。下面表格里的 `<id>` 同时接受短 ID 和完整 UUID，所以典型用法是 `multica issue list` → 复制 key → `multica issue get MUL-123`。需要完整 UUID 时给 `list` 加 `--full-id`。
</Callout>

| 命令 | 用途 |
|---|---|
| `multica issue list` | 列出议题（默认显示可复制的议题 key） |
| `multica issue get <id>` | 查看单条议题（接受议题 key 或 UUID） |
| `multica issue create --title "..."` | 创建新议题 |
| `multica issue update <id> ...` | 修改议题（状态、优先级、分配人等） |
| `multica issue assign <id> --agent <slug>` | 分配给智能体（立即触发任务） |
| `multica issue status <id> --set <status>` | 快捷改状态 |
| `multica issue search <query>` | 关键字搜索 |
| `multica issue runs <id>` | 查看议题上智能体跑过的任务 |
| `multica issue rerun <id>` | 给该议题当前的智能体分配人重新创建一条任务 |
...
| `multica attachment download <id>` | 下载议题 / 评论的附件 |
