...
这一页带你用 Docker 把 Multica 的**服务器**（后端 + 前端 + PostgreSQL）跑在自己的机器或服务器上。走完这一篇你的数据就完全在自己手里——包括 [工作区](/workspaces)、[议题](/issues)、[评论](/comments)、[智能体](/agents) 配置。
...
<Callout type="info">
**镜像还没发布？** 如果 `just selfhost` 报拉不到镜像，可能是你在某个未发布的版本标签上。切到稳定版本或直接从源码构建：`just selfhost-build`。
</Callout>
...
- [GitHub 集成](/github-integration) —— 连一个 GitHub App，让 PR 自动关联议题、merge 时自动转 Done
