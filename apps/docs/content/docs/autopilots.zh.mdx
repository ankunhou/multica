...
Autopilot 有两种执行模式，**建议从"先建议题模式"开始**：

- **先建议题模式**（`create_issue`）—— 默认，**推荐**。每次触发先在工作区里建一个议题（标题支持 `{{date}}` 这样的插值），再按分配流程把议题派给智能体。所有工作都落在议题看板上，历史、评论、状态和手动分配的议题完全一致。
- **直跑模式**（`run_only`）—— 不建议题，直接入队一个 `task`。看板上看不到这一次运行——只能在 Autopilot 的运行历史里看到。
...
- [**分配议题给智能体**](/assigning-issues) —— 一次性把议题指派给智能体
