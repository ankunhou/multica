---
title: GitHub 集成
description: 一次性连接 GitHub App，之后 PR 的分支名、标题或正文里写了议题编号（例如 MUL-123），就会自动挂到那个议题上——PR 合并时议题自动转 Done。
---

import { Callout } from "fumadocs-ui/components/callout";

在 **Settings → Integrations** 里一次性连一个 GitHub 账号或组织。之后任何 PR 只要分支名、标题或正文里出现议题编号（例如 `MUL-123`），就会**自动关联**到那个 [议题](/issues)，出现在议题详情页右侧的 **Pull requests** 区块里——PR 合并时，议题自动转 **Done**。
...
| **议题详情侧栏 → Pull requests** | 列出所有自动关联到该议题的 PR，含标题、仓库、状态（`Open` / `Draft` / `Merged` / `Closed`）和作者。点一行跳到 GitHub。 |
| **Webhook（后台）** | 每次 `pull_request` 事件触发：upsert PR 行 → 扫描里面的议题编号 →（重新）建立 link。幂等——重投 delivery 不会产生重复记录。 |
| **Merge 自动改 status** | PR 转 `merged` 时，所有已关联且状态不是 `Done` / `Cancelled` 的议题会被推到 `Done`。时间线里以 source 为 `github_pr_merged` 记录。 |
...
- 限定到本工作区——只匹配本工作区的 [议题 prefix](/workspaces)。前缀是 `MUL` 的工作区里，PR 出现 `FOO-1` 不会匹配，即使数字撞另一个议题也不会
- 自动去重——`Closes MUL-1, MUL-1` 只关联一次

一个 PR 里**可以同时引用多个议题**。比如 `Closes MUL-1, MUL-2`：PR 同时关联两个议题，合并时两个议题都会转 `Done`。

## Merge 自动转 Done 的规则

PR 的 `merged` 字段翻成 `true` 时，逐个评估关联的议题：
...
PR **关闭但没合并**——只更新 PR 卡片的状态为 `Closed`，议题状态不变。"关闭但不合并"语义因团队而异，Multica 不替用户做决定。

<Callout type="info">
状态变更的 actor 是 `system`。订阅了该议题的成员会收到 inbox 通知，和成员手动改状态时一致。
</Callout>
...
- **手动把 PR 关联到议题**——暂时没有这个 UI。如果你们的约定把编号放到 Multica 不扫的地方，请改放到 PR 标题或正文里
...
断开之后，已经镜像的 PR 行保留在数据库里——历史议题侧栏仍能显示当时关联的 PR，但来自这个 installation 的新 webhook 事件不再被接受。

## 权限和可见性

- **Connect / Disconnect** 需要工作区 **owner 或 admin**。普通成员能看到卡片描述但看不到 Connect 按钮
- **Pull requests** 侧栏对所有能看到该议题的成员可见——和议题详情页其他部分权限一致
...
之后在任意一个仓库开一个分支 / 标题 / 正文带本工作区议题编号的 PR——几秒内对应议题的详情页上就能看到 Pull requests 区块。
...
- **手动 link UI 暂未提供**——关联 PR 的唯一方法是把议题编号写到 PR 分支 / 标题 / 正文
...
- **同议题多 PR 时，merge 行为偏激进**——两个 PR 都引用 `MUL-123` 时，第一个 merge 就把议题转 Done。"等所有关联 PR 都解决再推进议题状态"的优化已经在做了

## 下一步

- [议题](/issues) —— PR 引用的议题编号（`MUL-123`）的来源
- [工作区](/workspaces) —— 工作区议题 prefix 的设置位置
